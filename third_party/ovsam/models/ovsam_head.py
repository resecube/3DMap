...
from third_party.ovsam.ext.sam import MaskDecoder
from third_party.ovsam.ext.sam.mask_decoder import MLP as SAMMLP
from third_party.ovsam.ext.meta.sam_meta import meta_dict, checkpoint_dict
from third_party.ovsam.utils.load_checkpoint import load_checkpoint_with_prefix
...
class OVSAMHead(BaseModule):
    ...
    def __init__(
        ...
        if self.with_label_token:
            ov_path = '/root/autodl-tmp/MM/third_party/ovsam/ckpt/RN50x16_LVISV1Dataset.pth'
    ...
    def predict_masks(
        ...
        if self.with_label_token:
            ...
            sematic_embed = roi_feats
            roi_feats = roi_feats[:, None] + 0 * cls_embed
            cls_pred = self.forward_logit(roi_feats)
        else:
            cls_pred = None
            sematic_embed = None # UPDATE
        return masks, None, cls_pred, sematic_embed # UPDATE
    ...
    def forward(
        ...
        masks, _, cls_pred,sematic_embed = self.predict_masks(
