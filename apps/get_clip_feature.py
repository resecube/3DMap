...
class CroppedImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        def get_cropped_image(mask, rgb):
            ...
            def mask2box_multi_level(mask, level, expansion_ratio):
                ...
                return (
                    max(0, left - x_exp),
                    max(0, top - y_exp),
                    min(shape[1], right + x_exp),
                    min(shape[0], bottom + y_exp),
                )
        ...
        input_images = [
            self.preprocess(pad_into_square(Image.fromarray(cropped_image)))
            for cropped_image in cropped_images
        ]
        input_images = torch.stack(input_images)
        return input_images, frame_id, mask_id


def get_clip_features(cfg, object_dict, dataset, model, preprocess):
    with torch.no_grad():
        (
            frame_id_list,
            mask_id_list,
        ...
        feature_dict = {}
        for images, frame_ids, mask_ids in tqdm(dataloader):
