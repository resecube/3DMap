...
def read_pridiction_npz(path):
    ...
    for i in range(num_instance):
        pred_info[path.split("/")[-1] + "_" + str(i)] = (
            {  # unique id of instance in all scenes
...
def get_gt_tensor(gt_ids, gt_instances):
    ...
    for label in gt_instances:
        ...
        for i, gt_instance_info in enumerate(gt_instances[label]):
            gt_tensor[:, i] = torch.from_numpy(
                gt_ids == gt_instance_info["instance_id"]
            )
        gt_tensor_dict[label] = gt_tensor
    return gt_tensor_dict 


def assign_instances_for_scan(pred_file, gt_file, opt):
    """
    ...
    for pred_mask_file in pred_info:
        ...
        intersection = torch.sum(
            gt_tensor & torch.from_numpy(pred_mask).cuda().reshape(-1, 1), dim=0
        )
        intersect_ids = (
            torch.nonzero(intersection).cpu().numpy().reshape(-1)
        )
        for gt_id in intersect_ids:
            ...
            gt2pred[label_name][gt_id]["matched_pred"].append(
                pred_copy
            )
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            _SPLITTER.join(
                [
                    str(x)
                    for x in [avgs["all_ap"], avgs["all_ap_50%"], avgs["all_ap_25%"]]
                ]
            )
            + "\n"
        )
...
def evaluate(pred_files, gt_files, output_file, opt):
    print("evaluating", len(pred_files), "scans...")
    matches = {}  
    for i in range(len(pred_files)):
        matches_key = os.path.abspath(gt_files[i])
        gt2pred, pred2gt = assign_instances_for_scan(pred_files[i], gt_files[i], opt)
        matches[matches_key] = {}
        matches[matches_key]["gt"] = gt2pred
        matches[matches_key]["pred"] = pred2gt
        sys.stdout.write("\rscans processed: {}".format(i + 1))
        sys.stdout.flush()
    ap_scores = evaluate_matches(matches, opt)
...
def eval(opt):
    ...
    pred_files = [
        f
        for f in sorted(os.listdir(opt.pred_path))
