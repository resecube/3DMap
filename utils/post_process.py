...
def merge_overlapping_objects(
    total_point_ids_list, total_bbox_list, total_mask_list, overlapping_ratio
):
    """
...
def filter_point(
    ...
    def count_point_appears_in_video(
        point_frame_matrix, point_ids_list, node_global_frame_id_list
    ):
        """
        ...
        for point_ids in point_ids_list:
            ...
            point_appear_in_video_nums.append(
                np.sum(point_appear_in_video_matrix, axis=1)
            )
    ...
    def count_point_appears_in_node(
        mask_list,
        node_frame_id_list,
        point_ids_list,
        mask_point_clouds,
        ...
        for frame_id, mask_id in mask_list:
            ...
            for i, point_ids in enumerate(point_ids_list):
                point_ids_within_object = np.where(np.isin(point_ids, mask_point_ids))[
                    0
                ]
                point_appear_in_node_matrixs[i][
                    point_ids_within_object, frame_id_in_list
                ] = True
                if len(point_ids_within_object) > largest_intersect:
                    object_id_with_largest_intersect, largest_intersect = i, len(
                        point_ids_within_object
                    )
                    coverage = len(point_ids_within_object) / len(point_ids)
            if largest_intersect == 0:
                continue
            object_mask_list[object_id_with_largest_intersect] += [
                (frame_id, mask_id, coverage)
            ]
    ...
    point_appear_in_video_nums, point_appear_in_node_matrixs = (
        count_point_appears_in_video(
            point_frame_matrix, point_ids_list, node_global_frame_id_list
        )
    )
    object_mask_list, point_appear_in_node_matrixs = count_point_appears_in_node(
        mask_list,
        node_frame_id_list,
        point_ids_list,
        mask_point_clouds,
    ...
    for i, (point_appear_in_video_num, point_appear_in_node_matrix) in enumerate(
        ...
        valid_point_ids = np.where(detection_ratio > cfg.point_filter_threshold)[0]
        ...
        filtered_bbox_list.append(
            [np.amin(pcld_list[i].points, axis=0), np.amax(pcld_list[i].points, axis=0)]
        )
...
def dbscan_process(pcld, point_ids, DBSCAN_THRESHOLD=0.1):
    ...
    labels = (
        np.array(pcld.cluster_dbscan(eps=DBSCAN_THRESHOLD, min_points=4)) + 1
    )  # -1 for noise
...
def find_represent_mask(mask_info_list, cfg):
    mask_info_list.sort(key=lambda x: x[2], reverse=True)
    return mask_info_list[: cfg.repre_num]


def export_class_agnostic_mask(cfg, class_agnostic_mask_list):
    ...
    np.savez(cfg.class_agnostic_file_save_path, **pred_dict)
    return
...
def post_process(
    ...
    object_dict = {}
    for i, (point_ids, mask_list) in enumerate(
        zip(total_point_ids_list, total_mask_list)
    ):
        object_dict[i] = {
            "point_ids": point_ids,
            "mask_list": mask_list,
            "repre_mask_list": find_represent_mask(mask_list, cfg),
        }

    # object_dict = export(dataset, total_point_ids_list, total_mask_list, cfg)
    # instances = export_instances(dataset, object_dict, scene_points, cfg)

    # return total_point_ids_list, total_mask_list, object_dict
    return object_dict
